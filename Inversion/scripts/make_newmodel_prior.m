function [ model, alpha ] = make_newmodel_prior( model, inverse_parameters, HV, PVr )
...
%5) Modify the uncertainties on the data (not implemented yet)
    ...
    switch action
        ...
        case 1 %birth
            ...
            modelN.(mode).value(end)         = rand(1)*range(inverse_parameters.limits.(mode));
            ...
            alpha = min([1 (1/diff(inverse_parameters.limits.(mode)))*exp(-(modelN.fit - model.fit)/2)]);
        ...
        case 2 %death
            ...
            alpha = min([1 diff(inverse_parameters.limits.(mode))*exp(-(modelN.fit - model.fit)/2)]);
