function [ model_fit, llh, HVfit, PVrfit, HV_model, PVr_model] = evaluate_models( model, HV, PVr, GVr, inverse_parameters )
%llh is log likelyhood
    ...
    if isempty(HV)
        ...
        [ HV_model, ~, ~ ] = get_rayleigh_values( model.interp.z/1000, ...
            model.interp.vs/1000, model.interp.vs.*model.interp.vpvs/1000, ...
            model.interp.rho, HV.frequency);
    ...
    if isempty(PVr)
        ...
        [ ~, PVr_model, ~ ] = get_rayleigh_values( model.interp.z/1000, ...
            model.interp.vs/1000, model.interp.vs.*model.interp.vpvs/1000, ...
            model.interp.rho, PVr.frequency);
