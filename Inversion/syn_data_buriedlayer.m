function [HV, ZJ0, model] = syn_data_buriedlayer( inverse_parameters )
    ...
    model.vs.z       = [ 0   10  25   35  49.9 50   100 ];%[ 1   10  70   75 119.9 120  200 ]
    model.vs.value   = [ 500 800 1000 500 500   2000 2000];%[ 1000 1000 1000 1000 1000  1000 1000]
    model.vs.n       = length(model.vs.value);
    model.vpvs.z     = [ 0   10  25   35  49.9 50   100 ];
    model.vpvs.value = [ 1.7 1.7 1.7  2.5 2.5  1.7  1.7];%[ 1.7 1.7 1.7 1.7 1.7  1.7  1.7];
    model.vpvs.n     =  length(model.vpvs.value);
    
    model = interpolate_model(model, inverse_parameters);
   
    [PVr.value,~,HV.value] = mat_disperse(diff(model.interp.z),model.interp.rho...
        ,model.interp.vpvs.*model.interp.vs,model.interp.vs,fPV,1e4);
