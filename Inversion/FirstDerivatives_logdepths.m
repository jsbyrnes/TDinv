...
f = 2.^(-1:3);
model.vs.z       = log2([ 0   10  25  35  49.9 50   100]);
model.vpvs.z     = log2([ 0   10  25  35  49.9 50   100]);
%model.vs.value   = [ 500  600 800 1000 1000     1500 1500];
%model.vpvs.value = [ 1.7 1.7 1.7 3.5 3.5       1.7  1.7];
model.vs.value   = [ 2000 2000 2000 2000 2000 2000 2000 ];
model.vpvs.value = [ 2    2    2    2    2    2    2 ];
...
inverse_parameters.log_depths    = 1; %log-spacing of depths?
model                            = interpolate_model(model, inverse_parameters);

%make labels
for k = 1:length(f)
   
    legendpowersoftwo{k} = [ '2^' num2str(log2(f(k))) ];
...
figure(1)
subplot(231)
plot(model.interp.vs, log2(model.interp.z))
...
ylabel('Depth, m')
subplot(234)
plot(model.interp.vpvs, log2(model.interp.z))
set(gca, 'YDir', 'reverse')
xlabel('VpVs ratio');
ylabel('Depth, m')
...
subplot(232)
plot(Vs1stPV(1:end-1, :), log2(model.interp.z(1:end-1)))
set(gca, 'YDir', 'reverse')
...
%figure(3)
subplot(233)
plot(Vs1stHV(1:end-1, :), log2(model.interp.z(1:end-1)))
set(gca, 'YDir', 'reverse')
...
subplot(235)
plot(VpVs1stPV(1:end-1, :), log2(model.interp.z(1:end-1)))
set(gca, 'YDir', 'reverse')
...
subplot(236)
plot(VpVs1stHV(1:end-1, :), log2(model.interp.z(1:end-1)))
set(gca, 'YDir', 'reverse')
