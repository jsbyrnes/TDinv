...
e12 = complex(zeros(2,2,length(cvs)));
e21 = complex(zeros(2,2,length(cvs)));
e22 = complex(zeros(2,2,length(cvs)));
du  = complex(zeros(2,2,length(thk)));
...
if om == 0
   ...
   ks2 = om2./cvs2;
   nus = sqrt(complex(k2-ks2));
   index = find(imag(-i*nus) > 0);
   nus(index) = -nus(index);
   gammas = nus/k;
   
   kp2 = om2./cvp2;
   nup = sqrt(complex(k2-kp2));
