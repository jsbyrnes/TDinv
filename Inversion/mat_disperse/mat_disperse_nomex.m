function [vr,varargout] = mat_disperse_nomex(thk,dns,cvp,cvs,freq,varargin)
...
varargout(1:8) = {0,0,0,0,0,0,0,0};

if length(varargin) < 2

    [vr, varargout{8}] = modal_nomex(freq,thk,dns,cvp,cvs,vrmin,vrmax);
    
elseif length(varargin) == 2
   
    [vr, varargout{8}] = modal_nomex(freq,thk,dns,cvp,cvs,vrmin,vrmax, varargin{2});
...
if (nargout >= 3)
   [z, r, dr] = disp_stress_nomex(freq,vr,thk,dns,cvs,cvp,Fz);
   varargout{1} = z; varargout{2} = abs(r(:,1,1,2))./abs(r(:,1,1,1));%arg 3 out is HV
end
